...
class DarkMode {
  constructor(el) {
    ...
    this.headerActions = document.querySelector(
  ...
  toggleColors() {
    ...
    if (this.dataSetup.includes("custom")) {
      this.sections.forEach(section => {
        ...
        const mappedTheme = this.themeMappings[defaultTheme];
        if (mappedTheme) { 
          section.classList.toggle(mappedTheme);
        } else {
          section.classList.add(defaultTheme);
        }
        if (section.matches('#sections > section:first-child') && mappedTheme) {
          this.header.classList.toggle(mappedTheme)
        } 
      });
      this.header.classList.toggle(this.darkTheme);
      this.mobileMenu.classList.toggle(this.darkTheme);
...
(function () {
  // Find All Instances
  let allInstances = document.querySelectorAll('[data-tool="dark-mode"], [data-wm-plugin="dark-mode"]');
